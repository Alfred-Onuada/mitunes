<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $successFileName = "../Logs/cpchangesuccess.txt";
        $successLogFile = fopen($successFileName, "a+");

        $failFileName = "../Logs/cpchangefail.txt";
        $failLogFile = fopen($failFileName, "a+");

        $id = $_GET['id'];
        
        $target_dir = "../Upload/Img/Profile/Cover/";
        $real_dir = "Upload/Img/Profile/Cover/";
        $file = $_FILES['newp']['tmp_name'];
        $selffile = basename($_FILES['newp']['name']);
        ...
        $check = getimagesize($_FILES["newp"]["tmp_name"]);
        if($check !== false) {
            $check = 1;
        } else {
            $check = 0;
        ...
        if ($check == 1) {
            ...
            // to delete the old cp to free up space
            ...
            $change = "UPDATE users SET cp = '$real_file' WHERE id = '$id'";
            ...
            if ($send) {
                ...
                if (move_uploaded_file($file, $target_file)) {
                    ...
                    $smsg = $fullName." successfully changed ".$sex." cover photo on ".date('D, d M Y H:i:s').PHP_EOL;
            ...
            $fmsg = $fullName." failed to change ".$sex." cover photo, because it didn't meet the parameters, on ".date('D, d M Y H:i:s').PHP_EOL;
